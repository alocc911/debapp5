...
function renderBody(text: string, terms?: string[], onLinkClick?: (id: string) => void) {  
  ...
  return parts.map((part, i) => {  
    ...
    return <Fragment key={i}>{highlight(part, terms)}</Fragment>;  
  });  
}  
...
export default function NodeCard({ id, data }: NodeProps<Data>) {  
  ...
  // Handle deselection  
  React.useEffect(() => {  
    if (data.isEditing && store.selectedNodeId !== id) {  
      handleBodyCancel();  
    }  
  ...
  }, [editBodyText, data.isEditing]);  

  const handleBodyDoubleClick = (e: React.MouseEvent) => {  
    e.stopPropagation();  
    if (!data.isEditing) {  
      store.updateNode(id, { isEditing: true });  
    }  
  };  
  ...
  const handleBodySave = (e?: React.MouseEvent) => {  
    e?.stopPropagation();  
    store.updateNode(id, { body: editBodyText, isEditing: false });  
    document.body.focus();  
  };  

  const handleBodyCancel = (e?: React.MouseEvent) => {  
    e?.stopPropagation();  
    store.updateNode(id, { isEditing: false });  
    setEditBodyText(data.body || '');  
    document.body.focus();  
  };  
